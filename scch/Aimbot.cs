                        ...
                        if (Engine.enemy[i].health > 0 && !Engine.enemy[i].bDormant && Engine.enemy[i].isEnemy && Engine.enemy[i].isVisible)
                        {
                           ...
                           //     MoveToVec(evec);
                                ...
                                for (int p=1; p<config.fSmooth; p++)
                                {
                                    ...
                                    MoveToVec(Engine.NormalizeAngle((Engine.LocalPlayer.angles -( Engine.LocalPlayer.ViewPunchAngle * 2.0f/(config.fSmooth - p))) + (evec - Engine.LocalPlayer.angles) / (config.fSmooth - p)));
                                    

                                    Thread.Sleep(10);
                                }

                               //    MoveToVec(Engine.NormalizeAngle(Engine.LocalPlayer.angles - Engine.LocalPlayer.ViewPunchAngle * 2.0f));
